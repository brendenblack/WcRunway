        ...
        public async Task RefreshUnits()
        {
            ...
            if (values != null && values.Count > 0)
            {
                ...
                foreach (var row in values)
                {
                    ...
                    var name = row.ReadColumnAsString(COL_NAME);
                    if (!string.IsNullOrWhiteSpace(name))
                    {
                        unit.Name = name;
                    }

                    unit.Key = row.ReadColumnAsString(COL_KEY);
                    unit.Identifier = row.ReadColumnAsString(COL_IDENTIFIER);
                    var description = row.ReadColumnAsString(COL_DESCRIPTION);
                    if (!string.IsNullOrWhiteSpace(description))
                    {
                        unit.Description = description;
                    }

                    var unlockSku = row.ReadColumnAsString(COL_UNLOCK_SKU);

                    Level level = new Level();
                    level.Number = row.ReadColumnAsInteger(COL_LEVEL);
                    level.Grade = row.ReadColumnAsInteger(COL_GRADE);
                    level.UpgradeCostMetal = row.ReadColumnAsInteger(COL_UPGRADE_COST_METAL);
