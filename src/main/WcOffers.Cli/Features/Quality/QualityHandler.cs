...
{
    public class QualityHandler : CommandLineHandler, ICommandLineHandler<QualityOptions>
    {
            ...
            foreach (var offerCode in opts.OfferCodes)
            {
                ...
                if (offer != null)
                {
                    Task.Run(async () => 
                    {
                        ...
                        logger.LogInformation("Issue for offer {} created with key {}", offer.OfferCode, issue.Key.Value);
                ...
                {
                    logger.LogError($"Unable to find an offer with code {offerCode}, no ticket was generated for it");
                }
            }
